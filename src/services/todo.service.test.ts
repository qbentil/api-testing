import { createTodo, deleteTodo, getTodo, getTodos, updateTodo } from '../services';
...
describe('Todo Service', () => {
    ...
    describe('updateTodo', () => {
        it('should update a todo', async () => {
            ...
            (TodoModel.findByIdAndUpdate as jest.Mock).mockResolvedValueOnce(updatedTodo);

            const result = await updateTodo('123', updatedTodo);
            expect(result).toEqual(updatedTodo);
            expect(TodoModel.findByIdAndUpdate).toHaveBeenCalledWith('123', updatedTodo, { new: true });
        });
    });
