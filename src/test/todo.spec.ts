...
describe('Todo Router', () => {
    ...
    describe('POST /create', () => {
        ...
        it("should return todo item as data when created", async () => {
            ...
            expect(response.body.data).toHaveProperty('description', requestBody.description);
            expect(Services.createTodo).toHaveBeenCalledWith(requestBody);
        });
    ...
    describe("GET /read", () => {
        it("should return todos with default pagination", async () => {
            ...
            (Services.getTodos as jest.Mock).mockImplementationOnce((query, pagination) => {
            ...
            expect(Services.getTodos).toHaveBeenCalledWith({}, { limit: 10, page: 1 });
        });
        ...
        it('should return filtered todos', async () => {
            const todos = [
                { title: 'Filtered Todo', description: 'Filtered Description', completed: true },
                { title: 'Unfiltered Todo', description: 'Unfiltered Description', completed: false },
            ];

            (Services.getTodos as jest.Mock).mockImplementationOnce((query) => {
                const { completed } = query;
                const filteredTodos = todos.filter((todo) => todo.completed == Boolean(completed));
            ...
            expect(Services.getTodos).toHaveBeenCalledWith({ completed: "true" }, { limit: 10, page: 1 });
        });
    ...
    describe("DELETE /delete/:id", () => {
        ...
        it('should throw error id is not provided', async () => {
            (Services.deleteTodo as jest.Mock).mockRejectedValueOnce(new Error('Invalid ID'));
